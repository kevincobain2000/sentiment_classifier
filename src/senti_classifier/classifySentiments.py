...
--Pulkit Kathuria
"""
__testrun__ = "http://www.jaist.ac.jp/~s1010205/code/index.html"
...
if __name__ == "__main__":
    ...
    if myarguments.SentiWN_path or not os.path.exists(pickles[0]):
        if not myarguments.SentiWN_path:
            print color('Run following commands FIRST', 'red')
        ...
        #print color('Success, Pickled Sentiwordnet to --> ', 'green'), pickles[0]
    if os.path.exists(pickles[0]) and myarguments.files:
        #print color('loading pickle...........', 'red')
        #print color('loading pickle successful', 'green')
        synsets_scores = pickle.load(open(pickles[0],'rb'))
        #print color('Training Bag Of Words...........', 'red')
        bag_of_words = train_bag_of_words()
        bag_of_words = classify_polarity(bag_of_words)
        #print color('Training Bag Of Words successful','green')
        ...
        for key in scorer.keys():
            ...
            #for key in scorer.keys():
                #print key,'is', color(sentiment.upper(), 'green'),'with score' ,score
